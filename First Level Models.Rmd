...
```{r}
set.seed(12345)
m1 <- step(glm(TARGET ~ ., data = default_train, family = "binomial"), direction = "forward")
summary(m1)

cat("Make predictions using Forward stepwise model\n")
default_pred_log <- predict(m1, default_test, type = "response")
default_pred_log1 <- ifelse(default_pred_log >= .5, 1, 0)
cm_log <- confusionMatrix(as.factor(default_pred_log1), as.factor(default_test$TARGET), positive = "1")
cm_log
...
default_pred_log2 <- ifelse(default_pred_log >= .05, 1, 0)
cm_log2 <- confusionMatrix(as.factor(default_pred_log2), as.factor(default_test$TARGET), positive = "1")
cm_log2
...
x <- model.matrix(TARGET ~ ., data = default_train)[, -1]
x_t <- model.matrix(TARGET ~ ., data = default_test)[, -1]
...
cm_logL <- confusionMatrix(as.factor(def_pred_log_L), as.factor(default_test$TARGET), positive = "1")
cm_logL
...
def_pred_log_L <- ifelse(def_pred_logL >= .05, 1, 0)
cm_logL <- confusionMatrix(as.factor(def_pred_log_L), as.factor(default_test$TARGET), positive = "1")
cm_logL
cm_logL$byClass["Sensitivity"]
...
s <- seq(from = 11, to = round(sqrt(nrow(default_train))) + 1, by = 2)
results <- data.frame(k = s, total = NA)
folds <- createFolds(default_train[,1], k = 10, returnTrain = TRUE)
...
for (i in seq_along(s)) {
  ...
  for (f in seq_along(folds)) {
    train_idx <- folds[[f]]
    valid_idx <- setdiff(seq_len(nrow(default_train)), train_idx)
    
    trainX <- default_train[train_idx, -1]
    trainY <- default_train[train_idx, 1]
    
    validX <- default_train[valid_idx, -1]
    validY <- default_train[valid_idx, 1]
...
default_pred_best <- knn(train = default_train[, -1], test = default_test[, -1], cl = default_train[, 1], k = best_k, prob = TRUE)
cm_knn <- confusionMatrix(as.factor(default_pred_best), as.factor(default_test[,1]), positive = "1")
cm_knn
cm_knn$byClass["Sensitivity"]
cm_knn$byClass["Specificity"]
cat("As seen in the output, our best KNN model uses k =", best_k, "and has a sensitivity of", cm_knn$byClass["Sensitivity"], "with a specificity of", cm_knn$byClass["Sensitivity"], "\n")
...
score <- c()
cat("Linear\n")
svm1 <- ksvm(as.factor(TARGET) ~ ., data = default_train, kernel = "vanilladot")
svmp <- predict(svm1, default_test)

cm_svm <- confusionMatrix(svmp, as.factor(default_test$TARGET), positive = "1")
score <- c(score, (cm_svm$byClass["Sensitivity"] * 3 + cm_svm$byClass["Specificity"]) )

cat("RBF\n")
svm1 <- ksvm(as.factor(TARGET) ~ ., data = default_train, kernel = "rbfdot")
svmp <- predict(svm1, default_test)
...
cat("Polynomial\n")
svm1 <- ksvm(as.factor(TARGET) ~ ., data = default_train, kernel = "polydot")
svmp <- predict(svm1, default_test)
...
svm1 <- ksvm(as.factor(TARGET) ~ ., data = default_train, kernel = "tanhdot")
svmp <- predict(svm1, default_test)
...
cat("Bessel\n")
svm1 <- ksvm(as.factor(TARGET) ~ ., data = default_train, kernel = "besseldot")
svmp <- predict(svm1, default_test)
...
cat("ANOVA\n")
svm1 <- ksvm(as.factor(TARGET) ~ ., data = default_train, kernel = "anovadot")
svmp <- predict(svm1, default_test)
...
cat("Spline\n")
svm1 <- ksvm(as.factor(TARGET) ~ ., data = default_train, kernel = "splinedot")
svmp <- predict(svm1, default_test)
...
svm_final <- ksvm(as.factor(TARGET) ~ ., data = default_train, kernel = kern, prob.model = TRUE)
svmp <- predict(svm_final, default_test)
cm_svm <- confusionMatrix(svmp, as.factor(default_test$TARGET), positive = "1")
cm_svm
...
svm_pred <- predict(svm_final, newdata = default_test, type = "probabilities")[, 2]
```

### Decision Tree

We will prune our hyperparameter, trials, using cross-validation.
...
for (i in seq_along(trials)) {
  t = trials[i]
  model <- C5.0(as.factor(TARGET) ~ ., data = default_train, trials = t)
  pred <- predict(model, default_test)
  cm <- confusionMatrix(as.factor(pred), as.factor(default_test$TARGET), positive = "1")
  
  results$total[i] <- cm$byClass['Sensitivity'] * 3 + cm$byClass["Specificity"]
}
best_t <- results$k[which.max(results$total)]

best_tree <- C5.0(as.factor(TARGET) ~ ., data = default_train, trials = best_t)
plot(best_tree)
plot(s, results$total, type = "l", xlab = "Trials", ylab = "3 * Sens + 1 * Spec", main = "Score vs Trials")
abline(v = best_t, col = "red", lwd = 2)
tree_pred <- predict(best_tree, default_test)
cm_tree <- confusionMatrix(as.factor(tree_pred), as.factor(default_test$TARGET), positive = "1")
cm_tree
...
dt_pred <- predict(best_tree, default_test, type = "prob")[, "1"]
```
...
for (i in seq_along(trees)) {
  t = trees[i]
  model <- randomForest(as.factor(TARGET) ~ ., data = default_train, ntree = i)
  pred <- predict(model, default_test)
  cm <- confusionMatrix(as.factor(pred), as.factor(default_test$TARGET), positive = "1")
...
best_rf <- randomForest(as.factor(TARGET) ~ ., data = default_train, n_tree = best_tree, importance = TRUE)
plot(best_rf)
plot(s, results$total, type = "l", xlab = "ntrees", ylab = "3 * Sens + 1 * Spec", main = "Score vs ntrees")
abline(v = best_tree, col = "red", lwd = 2)
tree_pred <- predict(best_rf, default_test)
cm_rf <- confusionMatrix(as.factor(tree_pred), as.factor(default_test$TARGET), positive = "1")
cm_rf
...
rf_pred <- predict(best_rf, default_test, type = "prob")[, "1"]
```

### ANN

Finally, we use two ANN models, one with hidden layers, and one without.
...
cat("First, no hidden layers\n")

score <- c()

set.seed(12345)
ann0 <- neuralnet(TARGET ~ ., data = default_train, hidden = 0, linear.output = FALSE, lifesign = "full", stepmax = 100000, threshold = 0.0001)

ann_results0 <- compute(ann0, default_test[, -which(names(default_test) == "TARGET")])
default_pred_ann0 <- ann_results0$net.result
default_bin_pred_ann0 <- ifelse(default_pred_ann0 >= .50, 0, 1)
cm_ann0 <- confusionMatrix(as.factor(default_bin_pred_ann0), as.factor(default_test$TARGET), positive = "1")
cm_ann0

score <- c(score, (cm_ann0$byClass["Sensitivity"] * 3 + cm_ann0$byClass["Specificity"]))

cat("Next, one hidden layer\n")
set.seed(12345)
ann1 <- neuralnet(TARGET ~ ., data = default_train, hidden = 1, linear.output = FALSE, lifesign = "full", stepmax = 500000, threshold = 0.0001)

ann_results1 <- compute(ann1, default_test[, -which(names(default_test) == "TARGET")])
default_pred_ann1 <- ann_results1$net.result
default_bin_pred_ann1 <- ifelse(default_pred_ann1 >= .50, 0, 1)
cm_ann1 <- confusionMatrix(as.factor(default_bin_pred_ann1), as.factor(default_test$TARGET), positive = "1")
cm_ann1
...
best_model <- get(paste0("ann", best_model_idx))
best_cm <- get(paste0("cm_ann", best_model_idx))
cat("ANN 1 is the no hidden layer, ANN 2 is one hidden layer\n")
...
for (i in seq_along(s)) {
  ...
  cm <- confusionMatrix(as.factor(ann_p), as.factor(default_test$TARGET), positive = "1")
...
cm_ann <- confusionMatrix(as.factor(ann_pred1), as.factor(default_test$TARGET), positive = "1")
cm_ann
